    ...
    <Link to={`/details/${id}`} className="p-6 shadow-md hover:scale-105 transition-transform duration-500 bg-[#FAFAFA] rounded-[20px]">
        <div>
            ...
            <div className="mt-4 flex flex-col gap-5">
                ...
                <div>
                    <h3 className="text-[24px] font-semibold font-secondary text-black mb-0">{name}</h3>
                    <span className="text-[16px] font-normal mt-2">{type}</span>
                </div>
                <div>
                    <h4 className="text-end text-[24px] text-[#5937E0] font-semibold">${price}</h4>
                    <span className="text-[14px] font-normal">per day</span>
                </div>
                </div>
            <div className="flex justify-between items-center">
                <div>
                    <img className="inline" src={gear_shift} alt="gear-shift" width={24} />
                <span className="text-[16px] font-normal ml-1">{transmission}</span>
                </div>
                <div>
        ...
        <button className="bg-[#5937E0] cursor-pointer mt-6 w-full px-7 py-3.5 rounded-xl text-white font-semibold text-[16px]">View Details</button>
    </Link>
  )
}
